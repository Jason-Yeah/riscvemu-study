/**
 * 解释执行
 * 当遇到不方便执行的指令或者指令复杂，指令有很少的变数时，使用解释执行。
 * 解释执行的核心是一个大switch语句，根据指令类型执行不同的代码。
 * （有段代码被反复执行，就认为是一个hot代码，用本机编译的方式用JIT执行方式）
 */

#include "../inc/riscvemu.h"
#include "../inc/interpret_utils.h"

// load byte: ld a1, 4   (a0) == rd = (i8)[imm + rs1]
//               rd  imm rs1
// static void func_lb(state_t *state, insn_t *insn)
// {
//     // 地址是64位，但数值是8位的，所以要进行符号扩展，先将立即数转换为64位有符号整数，再加上寄存器的值，最后从内存中读取一个字节的数据，并将其解释为一个8位有符号整数，存储到目的寄存器中。
//     u64 addr = state->gp_regs[insn->rs1] + (i64)insn->imm;
//     state->gp_regs[insn->rd] = *(i8 *)TO_HOST(addr); // lb
// }
...
void exec_block_interp(state_t *state)
{
    ...
    while(true)
    {
        ...
        state->gp_regs[zero] = 0; // zero寄存器始终为0，无论执行什么指令，都要确保它的值为0
        
        // control flow change/confilt
        // 在jmp指令和ecall把cont设置为true
        if (insn.cont) break;

        state->pc += insn.rvc ? 2 : 4; // 更新程序计数器，RVC指令占用2字节，普通指令占用4字节，根据指令类型更新pc的值
    }
}