...
#define QUADRANT(data) (data & 0x3) // RVC指令的象限，RVC指令的编码格式中，最低两位用于表示指令的象限，共有四个象限（0、1、2、3）

/**
 * normal types
 * OPCODE: 指令的操作码，(取5位)位于指令的第2到第6位，用于区分不同类型的指令
 * RD: 目的寄存器编号，(取5位)位于指令的第7到第11位，指定了指令操作的结果要存储到哪个寄存器中
 * FUNCT3: 指令的功能码，(取3位)位于指令的第12到第14位，用于进一步区分指令的功能，通常与OPCODE结合使用来确定指令的具体操作
 * RS1: 第一个源寄存器编号，(取5位)位于指令的第15到第19位，指定了指令操作的第一个源寄存器
 * RS2: 第二个源寄存器编号，(取5位)位于指令的第20到第24位，指定了指令操作的第二个源寄存器
 * FUNCT2: 指令的功能码，(取2位)位于指令的第25到第26位，用于进一步区分指令的功能，通常与OPCODE结合使用来确定指令的具体操作
 * RS3: 第三个源寄存器编号，(取5位)位于指令的第27到第31位，主要用于某些特定类型的指令，如浮点指令，指定了指令操作的第三个源寄存器
 * FUNCT7: 指令的功能码，(取7位)位于指令的第25到第31位，用于进一步区分指令的功能，通常与OPCODE结合使用来确定指令的具体操作
 * IMM116: 指令的立即数值，(取6位)位于指令的第26到第31位，通常用于计算地址、偏移量或作为操作数的一部分
*/
...
// {
//     return (insn_t) {
//         .imm = (i32)data >> 20, // 立即数是有符号的，需要进行符号扩展
//         .rs1 = RS1(data),
//         .rd = RD(data),
//     };
// }
