#ifndef INTERPRET_UTILS_H
#define INTERPRET_UTILS_H

#include "riscvemu.h"

FORCE_INLINE uint64_t mulhu(uint64_t a, uint64_t b) {
    uint64_t t;
    uint32_t y1, y2, y3;
...
FORCE_INLINE int64_t mulh(int64_t a, int64_t b) {
    int negate = (a < 0) != (b < 0);
    uint64_t res = mulhu(a < 0 ? -a : a, b < 0 ? -b : b);
    return negate ? ~res + (a * b == 0) : res;
}

FORCE_INLINE int64_t mulhsu(int64_t a, uint64_t b) {
    int negate = a < 0;
...
FORCE_INLINE u32 fsgnj32(u32 a, u32 b, bool n, bool x) {
    u32 v = x ? a : n ? F32_SIGN : 0;
    return (a & ~F32_SIGN) | ((v ^ b) & F32_SIGN);
}

FORCE_INLINE u64 fsgnj64(u64 a, u64 b, bool n, bool x) {
...
FORCE_INLINE u16 f32_classify(f32 a) {
    union u32_f32 uA;
    u32 uiA;
...
FORCE_INLINE u16 f64_classify(f64 a) {
    union u64_f64 uA;
    u64 uiA;
